...
def test_release_0_5_reproducible_resource_example():
    battle_length = BattleLengthAssumption(
        assumed_turns=3,
        end_type=BattleEndType.BROKEN_RANDOM_END,
    )

    starting_resources = HeroResourceState(
        remaining_might=3,
        remaining_will=4,
        remaining_fate=1,
    )
    ...
    after_combat_and_magic = apply_resource_allocations(
        resources=starting_resources,
        allocations=(
            ResourceAllocation(
    ...
    assert after_combat_and_magic == HeroResourceState(
        remaining_might=2,
        remaining_will=3,
        remaining_fate=1,
    )
